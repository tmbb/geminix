defmodule Geminix.V1beta.AsyncBatchEmbedContentRequest do
  ...
  alias Geminix.V1beta.InlinedEmbedContentRequest

  @doc """
  Start a new batch request.
  Raises in case of error.
  """
  @spec start!(binary(), list(InlinedEmbedContentRequest.t()), keyword()) :: t()
  def start!(model, inlined_embed_content_requests, opts \\ []) do
    {:ok, batch} = start(model, inlined_embed_content_requests, opts)
    batch
  end
  ...
  Start a new batch request.
  """
  @spec start(binary(), list(InlinedEmbedContentRequest.t()), keyword()) :: {:ok, t()} | {:error, any()}
  def start(model, inlined_embed_content_requests, opts \\ []) do
    ...
    full_url = @url_prefix
      <> "#{@api_version}/models/#{model}:asyncBatchEmbedContent"

    {:ok, %Geminix.V1beta.File{} = geminix_file} =
      Utils.with_tmp_path("-requests.jsonl", fn path ->
        jsonl_data =
          inlined_embed_content_requests
    ...
    batch_generate_content_request =
      %Geminix.V1beta.AsyncBatchEmbedContentRequest{
        batch: %Geminix.V1beta.EmbedContentBatch{
    ...
    handle_errors(start_batch_response, fn ->
      ...
      Geminix.V1beta.EmbedContentBatch.from_map(metadata)
    end)
  end
  ...
  def update_state(batch, opts \\ []) do
    ...
    handle_errors(response, fn ->
      ...
      Geminix.V1beta.EmbedContentBatch.from_map(batch, metadata)
    end)
  end
  ...
  def get_output(batch = %{output: output}, opts \\ [])
    ...
    inlined_responses =
      %Geminix.V1beta.InlinedEmbedContentResponses{
        inlined_responses:
          Enum.map(decoded, fn map ->
            {:ok, response} = Geminix.V1beta.InlinedEmbedContentResponse.from_map(map)
            response
          end)
      }
